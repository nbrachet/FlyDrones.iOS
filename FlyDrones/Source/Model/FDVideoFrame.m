...
- (instancetype)initWithFrame:(AVFrame *)frame {
    ...
    if (self) {
        ...
        self.luma = [self copyFrameData:frame->data[0]
                              frameSize:(int)frame->linesize[0]
                                  width:frame->width
                                 height:frame->height];
        self.chromaB = [self copyFrameData:frame->data[1]
                                 frameSize:(int)frame->linesize[1]
                                     width:(int)(frame->width / 2)
                                    height:(int)(frame->height / 2)];
        self.chromaR = [self copyFrameData:frame->data[2]
                                 frameSize:(int)frame->linesize[2]
                                     width:(int)(frame->width / 2)
                                    height:(int)(frame->height / 2)];
